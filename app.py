...
# =============================
# Utilitários e premissas (cálculos internos em moeda constante)
# =============================

INFLATION_ANNUAL = 0.04  # inflação usada internamente

@dataclass
class SalaryCurve:
    name: str
    annual_growth_pct: float  # entrada do sidebar (taxa aparente)
...
# =============================
# Simulador principal — permite riqueza NEGATIVA
# =============================
...
def main():
    ...
    with st.sidebar:
        ...
        annual_growth0_nom = st.slider(f"Crescimento anual inicial", -0.05, 0.25, curve.annual_growth_pct, 0.005)
        half_life      = st.slider("Meia-vida do crescimento (anos)", 1.0, 15.0, 5.0, 0.5)
        growth_floor_nom   = st.slider("Piso do crescimento anual no longo prazo", -0.02, 0.10, 0.02, 0.0025)
        ...
        initial_wealth = st.number_input("Patrimônio inicial (R$)", min_value=0.0, value=30000.0, step=1000.0)

        st.subheader("Premissas de retorno")
        selic_annual_nom = st.slider("Selic (a.a.)", 0.00, 0.20, 0.10, 0.0025, format="%0.2f")
        ...
        if uploaded is not None:
            try:
                ...
                st.success(f"Carregado {len(ibov_monthly_returns_nom)} retornos mensais do CSV.")
        ...
        if ibov_monthly_returns_nom is None:
            if use_monthly_editor:
                ...
                ibov_monthly_returns_nom = expand_annual_to_monthly_series(DEFAULT_IBOV_LAST10, years_each=1, noise=0.01)
                st.warning("Usando placeholders mensais (com leve ruído). Prefira CSV real.")

        st.subheader("Meta de aposentadoria")
        desired_income_real = st.number_input("Renda mensal desejada (R$)", min_value=0.0, value=30_000.0, step=1_000.0)
        retire_years_income = st.slider("Tempo de recebimento (anos)", 5, 60, 30)
        ...
        st.caption(f"Capital necessário: R$ {required_pv_sidebar:,.0f}")
        ...
        if enable_house:
            ...
            if "financiada" in house_mode:
                ...
                pmt_nom         = principal_nom * loan_m_nom / (1 - (1 + loan_m_nom) ** (-n_pay_nom)) if (loan_m_nom > 0 and n_pay_nom > 0) else (principal_nom / n_pay_nom if n_pay_nom > 0 else 0.0)
                st.info(f"Parcela estimada: R$ {pmt_nom:,.2f} | Total pago: R$ {pmt_nom*n_pay_nom:,.0f}")
    ...
    if run_btn:
        ...
        if pend > 0:
            st.markdown(
                f"**Nota:** {pend:.1f}% das simulações terminam com saldo devedor; descontado do patrimônio final."
            )
        ...
        with tab_overview:
            ...
            gp_df = pd.DataFrame({
                ...
                "Ações": eq_path_opt,
                "Renda Fixa": 1.0 - eq_path_opt,
            })
            gp_long = gp_df.melt(id_vars=["mês", "idade"], value_vars=["Ações", "Renda Fixa"],
                                 var_name="classe", value_name="peso")
            gp_chart = (
                alt.Chart(gp_long)
                .mark_line()
                .encode(
                    x=alt.X("idade:Q", title="Idade (anos)"),
                    y=alt.Y("peso:Q", title="Peso", axis=alt.Axis(format="%"), scale=alt.Scale(domain=[0, 1])),
                    color=alt.Color("classe:N", title="Classe"),
                    tooltip=[
                        alt.Tooltip("mês:Q", title="Mês"),
                        alt.Tooltip("idade:Q", title="Idade", format=".1f"),
        ...
        with tab_dist:
            st.subheader("Distribuição do valor final")
        ...
        with tab_assump:
            ...
            a3.metric("Selic (anual)", f"{selic_annual_real*100:0.2f}%")
        ...
        with tab_house:
            ...
            if not (enable_house and house_params["house_price"] > 0):
                ...
                st.subheader("% de simulações com prestação ativa por mês")
                if house_params.get("house_mode") == "divida":
                    ...
                    if paying_share["pct"].max() == 0:
                        ...
                        paying_chart = (
                            ...
                            .encode(
                                x=alt.X("mês:Q", title="Mês"),
                                y=alt.Y("pct:Q", title="% de simulações", scale=alt.Scale(domain=[0, 100])),
                ...
                if house_params.get("house_mode") == "avista":
                    ...
                    if first_purchase.empty:
                        st.info("Nenhuma simulação atingiu o valor da casa no horizonte.")
                    else:
                        ...
                        chart = (
                            ...
                            .encode(
                                x=alt.X("mês:Q", title="Mês"),
                                y=alt.Y("prob_acumulada:Q", title="Probabilidade acumulada", axis=alt.Axis(format="%"), scale=alt.Scale(domain=[0, 1])),
        ...
        with tab_sens:
            ...
            st.caption("Varia parâmetros ± e mede impacto em Prob(≥ meta) e Mediana final.")

        with tab_data:
            st.subheader("Retornos mensais do Ibovespa usados")
            st.dataframe(pd.DataFrame({"retorno_mensal": ibov_monthly_returns_real}), use_container_width=True)
            st.subheader("Parâmetros da simulação")
            st.json({
                "salário_base": base_salary,
                "crescimento_inicial_anual": annual_growth0_nom,
                "meia_vida_anos": half_life,
                "piso_crescimento_anual": growth_floor_nom,
                "pct_contribuição": contrib_pct,
                "alocacao": "por idade (otimizada)",
                "anos": years,
                "meses": months,
                "idade_atual": age0,
                "patrimonio_inicial": initial_wealth,
                "selic_anual": selic_annual_nom,
                "n_sims": n_sims,
                ...
                "meta": {
                    "renda_mensal": desired_income_real,
                    "anos": retire_years_income,
                    "capital_necessario": None if not np.isfinite(required_pv) else round(required_pv, 2),
                },
            })
